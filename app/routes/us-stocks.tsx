        ...
        <Grid item xs={12} md={3}>
            <Paper sx={{ p: 2 }}>
                ...
                <Typography variant="h6" color={getColor(totalDayChange)}>
                    {getSign(totalDayChange)}${Math.abs(totalDayChange).toLocaleString(undefined, { maximumFractionDigits: 2 })} ({totalDayChangePercent.toFixed(2)}%)
                </Typography>
            </Paper>
        </Grid>
        <Grid item xs={12} md={3}>
            <Paper sx={{ p: 2 }}>
                ...
                <Typography variant="h6" color={getColor(totalChange)}>
                    {getSign(totalChange)}${Math.abs(totalChange).toLocaleString(undefined, { maximumFractionDigits: 2 })} ({totalChangePercent.toFixed(2)}%)
                </Typography>
            </Paper>
        </Grid>
      </Grid>
                  ...
                  >
                    ...
                    <TableCell align="right">
                        ...
                        <Typography variant="caption" color="text.secondary" sx={{ fontVariantNumeric: 'tabular-nums' }}>
                            Avg: ${row.averagePrice.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                        </Typography>
                    </TableCell>

                    <TableCell align="right">
                        <Typography variant="body2" sx={{ fontVariantNumeric: 'tabular-nums' }}>
                            ${row.lastPrice?.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                        </Typography>
                        <Box sx={{ color: getColor(dayChangeVal), display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                             <Typography variant="caption" fontWeight="medium" sx={{ fontVariantNumeric: 'tabular-nums' }}>
                                {getSign(dayChangeVal)}${Math.abs(dayChangeVal).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                             </Typography>
                    ...
                    <TableCell align="right">
                         <Box sx={{ color: getColor(returns), display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                             <Typography variant="body2" fontWeight="medium" sx={{ fontVariantNumeric: 'tabular-nums' }}>
                                {getSign(returns)}${Math.abs(returns).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                             </Typography>
